...
#define NFS_INODE_PER_FILE      1 //一个文件对应一个inode
#define NFS_DATA_PER_FILE       15//采用直接索引的方式，一个文件最多使用15个数据块
...
/**
 * 这里我们设定一个逻辑块里面最多可以存储16个inode，也就是一个inode需要64B大小
 * 一个文件最多使用16个数据块，那么4MB最多可以存储4MB/(16KB+64B)=255个文件
 * 255个文件需要255个索引，一个索引对应64B大小，所以索引节点区最多需要16个逻辑块就可以存储全部索引节点
 * 那么一个用于超级块区，两个位图，16个索引节点，那么数据块区有4096-1-1-1-16=4077个逻辑块
 */
#define NFS_INODE_PER_BLK       16     // 一个逻辑块能放16个inode
#define NFS_SUPER_BLKS          1 //超级块
#define NFS_INODE_MAP_BLKS      1//索引节点位图
#define NFS_DATA_MAP_BLKS       1 //数据位图
#define NFS_INODE_BLKS          16    // 4096/16/16(磁盘大小为4096个逻辑块，维护一个文件需要16个逻辑块即一个索引块+15个数据块)
